...
import org.virtuslab.iskra.{Agg, Col}
import org.virtuslab.iskra.UntypedOps.typed
import org.virtuslab.iskra.types.*
import org.virtuslab.iskra.types.DataType.{Nullable, NumericOptType}
...
object Aggregates:
  def sum(
      using
      agg: Agg
  ): Sum[agg.type] = new Sum(agg)
  def max(
      using
      agg: Agg
  ): Max[agg.type] = new Max(agg)
  def min(
      using
      agg: Agg
  ): Min[agg.type] = new Min(agg)
  def avg(
      using
      agg: Agg
  ): Avg[agg.type] = new Avg(agg)
