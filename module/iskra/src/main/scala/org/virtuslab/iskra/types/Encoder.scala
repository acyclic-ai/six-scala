...
import org.virtuslab.iskra.MacroHelpers.TupleSubtype
import org.virtuslab.iskra.{:=, Name}
...
object StructEncoder:
  ...
  private def getColumnSchemaType(
      using
      quotes: Quotes
  )(subcolumnInfos: List[ColumnInfo]): Type[?] =
  ...
  private def getSubcolumnInfos(elemLabels: Type[?], elemTypes: Type[?])(
      using
      Quotes
  ): List[ColumnInfo] =
    elemLabels match
  ...
  def fromMirrorImpl[A: Type](
      using
      q: Quotes
  ): Expr[StructEncoder[A]] =
    import q.reflect.*

    Expr
      .summon[Mirror.Of[A]]
      .getOrElse(throw new Exception(s"Could not find Mirror when generating encoder for ${Type.show[A]}")) match
      case '{
        ...
        val structFieldExprs = subcolumnInfos.map { info =>
          '{
            sql.types
              .StructField(${ Expr(info.labelValue) }, ${ info.encoder }.catalystType, ${ info.encoder }.isNullable)
          }
        }
              ...
              case '[t] =>
                '{
                  ${ info.encoder }
                    ...
                    .encode(${ Select.unique(value.asTerm, info.labelValue).asExprOf[t] })
                }
          }
  ...
  given optFromMirror[A](
      using
      encoder: StructEncoder[A]
  ): (Encoder[Option[A]] { type ColumnType = StructOptType[encoder.StructSchema] }) =
